...
void dataG::getGPUs() {
	for (string ele : stdAss) {
		...
		for (int t = 0; t < sleft.size(); t++) {
			if (sleft[t] == '*') { ind[0]++; continue; }
			lop = sleft.substr(t, sleft.size()); break;
		}
		for (int t = 0; t < sright.size(); t++) {
			if (sright[t] == '*') {	ind[1]++; continue; }
			if (sright[t] == '&') { ind[2]++; continue; }
			rop = sright.substr(t, sright.size()); break;
		}
		if (ind[0]!= 0 && ind[1] != 0) {
...
void dataG::TSandSSop() {
	for (int i = 0; i < gpus.size(); i++) {
		for (int j = 0; j < gpus.size(); j++) {
			if (isSS(gpus[i], gpus[j]))SScomp(gpus[i], gpus[j]);
			if (isTS(gpus[i], gpus[j]))TScomp(gpus[i], gpus[j]);
		}
	}
}
